              ...
              <div class="card p-4 position-relative">
                  <div class="dropdown position-absolute top-0 end-0 mt-2 me-2">
                      ...
                      <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuButton">
                        @if ($test !== null)
                        <li><a class="dropdown-item" href="{{ route('adaptive.test.do', ['test' => 'post-test','classroomID' => $classroomID]) }}">Kerjakan post test</a></li>

                        @endif
                          <li><a class="dropdown-item" href="{{ route('adaptive.classroom.out', ['classroomID' => $classroomID]) }}" onclick="return confirm('Apakah anda yakin ingin keluar dari kelas ini?')">Keluar kelas</a></li>
                      </ul>
                  </div>
                  @if ($test !== null)
                      <h3>{{ $class->name }}</h3>
                      {{-- @if (isset($subjects[0]->subject->subject))     --}}
                          ...
                          <a href="{{ route('adaptive.classroom.modul', ['subjectID'=> $subject->subject_id]) }}" class="btn btn-outline-secondary text-left my-2 p-2" onClick="startTimer({{ $subject->id }})" data-subject-id="{{ $subject->id }}">
                  ...
                  @else
                      ...
                      <form action="{{ route('adaptive.save_modules') }}" method="post">
                        @csrf
                        @method('PUT')
...
<script>
  ...
  function fetchSubjects() {
    ...
    fetch(`/adaptive/get-modules?start=${startSubject}&end=${endSubject}`)
...
function startPretest(startSubject, endSubject) {
      ...
      if (confirmed) {
          const url = "{{ route('adaptive.test.do', ['test' => 'pre-test', 'classroomID' => $classroomID]) }}";
